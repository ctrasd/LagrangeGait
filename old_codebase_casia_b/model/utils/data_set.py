...
class DataSet_lmdb(tordata.Dataset):
    def __init__(self, seq_dir, label, seq_type, view, cache, resolution , cut=False,lmdb_path=None,index_dict=None):
        self.seq_dir = seq_dir
        self.view = view
        self.seq_type = seq_type
        self.label = label
        ...
        datalmdb_path = lmdb_path
        if lmdb_path is not None:
            self.data_lmdb = lmdb.open(datalmdb_path, readonly=True)
       
    def __getitem__(self, index):
        with self.data_lmdb.begin() as txn:
            imgs=txn.get(('img_'+str(index)).encode())
            view=txn.get(('id_'+str(index)).encode()).decode()
            seq_type=txn.get(('type_'+str(index)).encode()).decode()
            seq_id=txn.get(('id_'+str(index)).encode()).decode()

            image_buf = np.frombuffer(imgs, dtype=np.uint8)
            #print(image_buf.shape)
            image_buf=np.reshape(image_buf,(-1,64,64))
            image_buf=image_buf[:, :, self.cut_padding:-self.cut_padding].astype(
                'float32') / 255.0
        return image_buf, 0, view,seq_type,seq_id
