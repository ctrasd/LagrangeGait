...
from .network import TripletLoss, SetNet,SetNet_angle_motion
from .network import vgg_c3d
from .utils import TripletSampler,Logger
...
class Model:
    def __init__(self,
        ...
        print(train_pid_num)
        self.m_resnet = vgg_c3d.C3D_VGG_angle_emb_motion(num_classes=self.train_pid_num)#C3D_VGG_angle_emb_diff(num_classes=self.train_pid_num)
        #self.m_resnet=SetNet_angle_motion(256)#C3D_VGG_angle_emb_motion()#C3D_VGG_angle_emb()#transview()#transview_pure()
    ...
    def fit(self):
        ...
        for seq, view, seq_type, label, batch_frame in train_loader:
            ...
            #triplet_feature,angle_prob,ce_feature = self.m_resnet(seq)
            triplet_feature,angle_prob = self.m_resnet(seq)
            ...
            '''
            n, hpp, classn = ce_feature.size()
            ce_feature = ce_feature.view(n*hpp, -1)
            ...
            id_loss=self.ce_loss(ce_feature,targets)
            '''
            ...
            loss=loss+0.03*ce_loss#+0.1*id_loss
            ...
            if self.restore_iter % 70000 == 0:
                self.optimizer = optim.Adam([
                    {'params': self.m_resnet.parameters()}], lr=0.00005,weight_decay=self.weight_decay)  
            if self.restore_iter % 400000 == 0:
                self.optimizer = optim.Adam([
                    {'params': self.m_resnet.parameters()}], lr=0.00005)  

            if self.restore_iter % 100 == 0:
                print('iter {}:'.format(self.restore_iter), end='')
                print(', hard_loss_metric={0:.8f}'.format(np.mean(self.hard_loss_metric)), end='')
                print(', full_loss_metric={0:.8f}'.format(np.mean(self.full_loss_metric)), end='')
                #print(', id_loss_metric={0:.8f}'.format(id_loss), end='')
    ...
    def transform(self, flag, batch_size=1):
        with torch.no_grad():
            ...
            for i, x in enumerate(data_loader):
                ...
                #outputs,angle,ce_feature = self.m_resnet(seq)
                outputs,angle = self.m_resnet(seq)
        ...
        print(len(feature_list),feature_list[0].shape)
